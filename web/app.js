            ...
            return {
                ...
                execute: function (tasks) {
                    ...
                    return Rx.Observable
                        ...
                        .flatMap(function () {
                            return execReport$.filter(x => x.origin === id).takeUntil(complete$.filter(x => x.origin === id));
                        }).map(x => x.data);
                }
            }
        })
...
<ul ng-show="!ctrl.current.runnables.length">
    <li ng-repeat="task in ctrl.tasks track by $index">{{task}} 
...
`,
                controllerAs: 'ctrl',
                restrict: 'E',
                controller: ['$timeout', '$scope', 'Tasks', function ($timeout, $scope, Tasks) {
                    var ctrl   = this;
                    ctrl.tasks = [];
                    ctrl.current = {
                        runnables: [],
