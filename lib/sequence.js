...
module.exports.createSequence = function (tasks, input, config) {
    ...
    function flatten(initial, items) {
        ...
        return items.reduce((all, item) => {
             ...
             * but did have the compat flag, load the compat function for it
             */
...
function loadModules(input, modules, item) {
     ...
     *
     *  -> ['test/fixtures/tasks/simple.js:dev', 'test/fixtures/tasks/simple.js:other']
     */
    if (item.alias) {
        lookup = item.alias;
    }
 ...
 * as compat tasks so that we can display the correct headings
 * in the summary
 * @param sequence
 * @returns {*}
 */
