    ...
    const stream$ = Rx.Observable
        ...
        .flatMapLatest(() => {
            return watchers
                ...
                .do(obj => debugWatcher(`~ [id:${obj.event.watcherUID}] [${obj.event.event}] ${obj.event.path}`))
                /**
                 ...
                 */
                .flatMap(x => {
                    const runner = getTaskRunner(x);
                    /**
                     * On successful completion of tasks,
                     * reset the tracker so that this namespace is
                     * no longer running
                     */
                    runner.subscribeOnCompleted(() => {
                        x.tracker[x.event.namespace].running = false;
                        tracker.onNext(x.tracker);
                    });
                    return runner;
                });
