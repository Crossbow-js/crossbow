    ...
    opts.pkg.crossbow.watch.tasks.forEach(function (item) {
        ...
        Object.keys(item).forEach(function (key) {
            ...
            if (typeof item[key] === "string") {
                ...
                bs.watch(watchPatterns, watchConfig, function () {
...
module.exports = function (cli, opts) {
    ...
    var before   = opts.pkg.crossbow.watch.before || [];
    opts.handoff = true;

    logger.info('running {cyan:%s} before watcher starts', before);

    if (before.length) {
        _cli({input: ['run'].concat(before)}, opts)
            .then(function () {
                logger.info('{ok: } {cyan:%s} completed', before);
                runWatcher(cli, opts);
            }).catch(function (err) {
                console.log(err.message);
                console.log(err.stack);
