...
module.exports = function runWatcher (cli, input, config) {
    ...
    completedTasks$
        ...
        .withLatestFrom(stateManager.state, (task, state) => {
            task.value.summary = state[task.value.event.watcherUID];
            return task.value;
        })
        .subscribe(obj => {
            logger.info(`{green:√} {gray:[id:${obj.event.watcherUID}] [${obj.event._id}] ${obj.summary.runTime}ms}`);
        }, (e) => {
