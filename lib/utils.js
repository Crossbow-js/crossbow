...
utils.retrieveConfig = function (opts, flags) {

    var maybePath = resolve(opts.cwd, "./package.json");
    var pkg;

    try {
        pkg = require(maybePath);
        if (pkg.crossbow) {
            return pkg.crossbow;
        } else {
            return tryFileLookup(opts);
        }
    } catch (e) {
        return tryFileLookup(opts);
    }

    function tryFileLookup(opts) {
        if (typeof flags.config === 'string') {
            var maybe = resolve(opts.cwd, flags.config);
            if (exists(maybe)) {
