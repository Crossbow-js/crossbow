...
class TaskResolver  {
    ...
    flatTask (taskName, parent) {
        ...
        const moduleError    = (locatedModules.length === 0 && childTasks.length === 0) ? [{type: 'MODULE_NOT_FOUND'}] : [];
        ...
        const subTaskErrors = (() => {
            return subTaskItems.reduce((all, name) => {
                ...
                const match = objPath.get(this.input, ['config'].concat(baseTaskName, name));
                if (match === undefined) {
                    return all.concat({
        ...
        return createTask({
            ...
            valid:    true,
            parent:   parent,
            errors:   moduleError.concat(subTaskErrors)
        });
    }
