...
function requireModule(item) {
    ...
    return { taskItems: taskItems, completed: completed };
}
...
function loadModules(input, modules, item, parentTaskName) {
    ...
    var config = objPath.get(input, 'config', {});

    if (!item.subTasks.length) {
        var topLevelOpts = objPath.get(input, ['config', item.taskName], {});
        return {
    ...
    return item.subTasks.map(function (subTask) {
        var subTaskOptions = objPath.get(input, ['config', item.taskName, subTask], {});
        return {
            seq: requireModule(modules[0]),
