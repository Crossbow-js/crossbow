...
function runWatcher(cli, input, config, cb) {
    ...
    })['do'](onSwitch.onNext.bind(onSwitch)).subscribe();

    var watcherStream = watchers.filter(function (x) {
        return x.event === 'change';
    }).map(function (x) {
        x.tasks = splitTasks(x.tasks);
        return x;
    });

    var pauser = onSwitch.flatMapLatest(function (x) {
        return watcherStream.takeUntil(offSwitch);
    });

    pauser['do'](function (x) {
        return offSwitch.onNext(true);
    })['do'](runCommandAfterWatch).subscribe();
